-- Task 7 forward migration (review-only; do not execute in this environment)

alter table public.invoice_scan_batches
  add column if not exists operation_id text;

create unique index if not exists invoice_scan_batches_user_id_operation_id_key
  on public.invoice_scan_batches (user_id, operation_id)
...
begin
  ...
  -- Preserve Task 6 validation semantics before replay acceptance by validating
  -- all items/pages using the same explicit checks and error messages.
  for v_item in
    select value
  ...
  select b.id, b.extraction_mode
  into v_existing_batch_id, v_existing_mode
  ...
  if v_existing_batch_id is null then
    ...
    exception
      when unique_violation then
        -- Concurrent retry: resolve to the winner row instead of leaking raw unique_violation.
        select b.id, b.extraction_mode
        into v_existing_batch_id, v_existing_mode
        from public.invoice_scan_batches b
        where b.user_id = v_user_id
  ...
  -- No existing operation: continue with original Task 6 persistence flow unchanged.
  for v_item in
    select value
    from jsonb_array_elements(p_items)
  loop
...
-- Remove old signature so Task 7 idempotency cannot be bypassed.
drop function if exists public.persist_invoice_scan_batch_atomic(text, jsonb);

-- Rollback SQL (review-only; do not execute in this environment)

-- Remove Task 7 signature first.
drop function if exists public.persist_invoice_scan_batch_atomic(text, jsonb, text);

-- Restore exact Task 6 function and permissions.
create or replace function public.persist_invoice_scan_batch_atomic(
  p_extraction_mode text,
  p_items jsonb
)
...
begin
  ...
  insert into public.invoice_scan_batches (
    user_id, status, extraction_mode, error_message
  )
  values (
    v_user_id, 'capturing', v_mode, ''
  )
  returning id into v_batch_id;
...
revoke all on function public.persist_invoice_scan_batch_atomic(text, jsonb) from public;
revoke all on function public.persist_invoice_scan_batch_atomic(text, jsonb) from anon;
grant execute on function public.persist_invoice_scan_batch_atomic(text, jsonb) to authenticated;

-- Roll back schema additions.
drop index if exists public.invoice_scan_batches_user_id_operation_id_key;
alter table public.invoice_scan_batches drop column if exists operation_id;
